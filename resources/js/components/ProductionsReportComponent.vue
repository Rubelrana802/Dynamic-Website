                        ...
                        <form class="form-inline" @submit.prevent="custom_filtered_productions" autocomplete="off">
                            <div class="input-group mr-sm-2 input-group-sm">
                                ...
                                <select name="product_id" v-model="input.product_id" class="form-control">
                                    ...
                                    <option v-for="product in products" :value="product.id">{{ product.product_name }} ({{ product.category.category_name }})</option>
                                </select>
                            </div>
                            <div class="input-group mr-sm-2 input-group-sm">
                                <div class="input-group-prepend">
...
<script>
    export default {
        ...
        methods: {
            today_productions() {
                axios.get(baseURL + '/admin/productions/reports/today-productions')
                    .then(response => {
            ...
            custom_filtered_productions(){
                axios.post(baseURL + '/admin/productions/reports/custom-filtered-productions', this.input)
                    .then(response => {
                        this.productions = response.data
                    })
                    .catch(errors => {
            ...
            get_products() {
                axios.get(baseURL + '/admin/productions/published-products')
                    .then(response => {
